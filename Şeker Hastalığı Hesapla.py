...
def deneme(a, b, c, d, e, f, g, h):
    diabetes = pd.DataFrame(np.array([
    ...
    # 1 - LogisticRegression ALGORİTMASI
    df = diabetes.copy()
    df = df.dropna()
    y = df["Outcome"]
    ...
    girdi = np.transpose(girdi)

    hastalikdurumu = []
    hastalikdurumu.append(loj_model.predict_proba(girdi))
    #############################################################################################################################
    # 2 - Gaussian Naive Bayes ALGORİTMASI
    df = diabetes.copy()
    df = df.dropna()
    y = df["Outcome"]
    X = df.drop(['Outcome'], axis = 1)
    ...
    hastalikdurumu.append(nb_model.predict_proba(girdi))
    ...
    hastalikdurumu.append(knn_tuned.predict_proba(girdi))
    #############################################################################################################################
    #4 - SVC
    df = diabetes.copy()
    df = df.dropna()
    ...
    #Tahmin
    svc_tuned = SVC(probability = True, kernel = "linear", C = 5).fit(X_train, y_train)
    ...
    hastalikdurumu.append(svc_tuned.predict_proba(girdi))
    ############################################################################################################################
    ...
    hastalikdurumu.append(mlpc_tuned.predict_proba(girdi))
    #############################################################################################################################
    # 6 - CART ALGORİTMASI
    df = diabetes.copy()
    df = df.dropna()
    ...
    hastalikdurumu.append(cart_tuned.predict_proba(girdi))

    #############################################################################################################################
    # 7 - Random Forest ALGORİTMASI
    df = diabetes.copy()
    df = df.dropna()
    y = df["Outcome"]
    X = df.drop(['Outcome'], axis = 1)
    ...
    hastalikdurumu.append(rf_tuned.predict_proba(girdi))
    #############################################################################################################################
    ...
    hastalikdurumu.append(gbm_tuned.predict_proba(girdi))


    #############################################################################################################################

    #SONUÇLARI GOSTER
    modeller = [loj_model, nb_model, knn_tuned, mlpc_tuned, cart_tuned, rf_tuned, gbm_tuned]
    dogruluk = []
    isimler = []
    for model in modeller:
        isimler.append(model.__class__.__name__)
        y_pred = model.predict(X_test)
        dogruluk.append(accuracy_score(y_test, y_pred))

    print(dogruluk)
    print(isimler)
    index_min = np.argmax(dogruluk)
    print(index_min)

    basarimsonucu = str("En İyi Başarım: {:.2%}".format(dogruluk[index_min]))
    hastaolmama =  str("Hasta Olmama Yüzdeniz: {:.2%}".format(hastalikdurumu[0][0][0]))


    return f"{basarimsonucu} \nAlgoritma: {isimler[index_min]} \n{hastaolmama}"
